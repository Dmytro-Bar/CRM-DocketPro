...
> Версія: 1.3 · Дата: 14.06.2026 · Автор: Дмитро Барабин
...
│
├── _web/                           ← Web-застосунок
│   ├── main.py                     ← FastAPI app, монтування роутерів, фоновий цикл НБУ
...
│   │
│   ├── routers/
│   │   ├── dashboard.py            ← Дашборд + KPI + НБУ-блок
│   │   ├── clients.py              ← Клієнти CRUD
│   │   ├── contracts.py            ← Договори CRUD + прив'язка до курсу НБУ
│   │   ├── invoices.py             ← Рахунки CRUD + генерація PDF
│   │   ├── acts.py                 ← Акти CRUD + генерація PDF
│   │   ├── expenses.py             ← Витрати CRUD + категорії + мультивалютність
│   │   ├── payments.py             ← Звірка з Monobank
│   │   ├── app_payments.py         ← Позадоговірні надходження
│   │   ├── lyqpay.py               ← LiqPay інтеграція
│   │   ├── taxes.py                ← Податки ФОП (ЄП 5% + ВЗ 1%, наростаючим)
│   │   └── doc_templates.py        ← Управління шаблонами DOCX
│   │
│   ├── templates/                  ← Jinja2 HTML-шаблони
│   │   ├── base.html               ← sidebar + глобальний JS (initSortable, drawer)
│   │   ├── dashboard.html          ← КПІ + НБУ-блок + графік
│   │   ├── clients.html / client_form.html
│   │   ├── contracts.html / contract_form.html
│   │   ├── expenses.html           ← витрати + управління категоріями + edit drawer
│   │   ├── payments.html
│   │   ├── app_payments.html
│   │   ├── lyqpay.html
│   │   ├── taxes.html              ← ФОП-звіт по податках
...
| `status` | TEXT | `Активний` / `Призупинено` / `Завершено` / `Чернетка` |
...
| `nbu_rate` | REAL | Базовий курс НБУ USD/UAH на дату укладення |
| `nbu_tracking` | INTEGER | 1 = відстежувати курс НБУ, 0 = ні |
| `nbu_threshold_pct` | REAL | Поріг відхилення курсу % (за замовчуванням 5.0) |
...
|---|---|---|
| `id` | INTEGER PK | Auto |
| `pay_date` | TEXT | Дата (dd.mm.yyyy) |
| `amount` | REAL | Сума |
| `currency` | TEXT | UAH |
| `description` | TEXT | Опис |
| `source` | TEXT | `lyqpay` / `Приват24` / `Банк` / `Готівка` / `Інше` |

### Таблиця `expense_categories`

| Колонка | Тип | Опис |
|---|---|---|
| `id` | INTEGER PK | Auto |
| `name` | TEXT UNIQUE | Назва категорії витрат |

Заповнюється стандартними категоріями при першому `init_db()`. Редагується через UI (`/expenses`).

### Таблиця `nbu_rate_cache`

| Колонка | Тип | Опис |
|---|---|---|
| `id` | INTEGER PK | Auto |
| `fetched_ts` | INTEGER | Unix timestamp отримання |
| `rate` | REAL | Курс USD/UAH |
| `rate_date` | TEXT | Дата курсу від НБУ (dd.mm.yyyy) |

Кеш оновлюється автоматично раз на добу фоновим asyncio-завданням. Вік кешу перевіряється при кожному запиті (6-годинний TTL для on-demand, добовий для background refresh).
...
- **Блок НБУ** (якщо є договори з прив'язкою до курсу):
  - Поточний курс USD/UAH (великий моноширинний шрифт) + дата оновлення
  - Таблиця всіх tracked-договорів: базовий курс, поточний курс, δ%
  - Підсвічування рядків де δ% ≥ порогу (попередження про перегляд ціни)
...
**API:** `GET /api/nbu-rate` → `{"rate": float, "date": str, "from_cache": bool}`
...
- Підтримка слешів у номері договору (`15/03/2025`) — FastAPI path-параметр `{contract_no:path}`
...
**Нумерація:** `RAH-{ddmmyyyy}-{NNN}` — автоматично, NNN = MAX існуючого порядкового номера за день + 1 (стійко до видалення записів)
...
**Нумерація:** `ACT-{ddmmyyyy}-{NNN}` — MAX-based, стійко до видалення записів
...
**CRUD:** список + форма додавання + inline-редагування через drawer + видалення

**Категорії:** зберігаються в таблиці `expense_categories`, редагуються через панель у правій частині сторінки (додати / видалити). Стандартні категорії сіяються при `init_db()`.

**Мультивалютність:** 
- При введенні: `amount` + `currency` + `exchange_rate` → `amount_uah`
- У таблиці: головна сума в гривнях + підрядок з оригінальною сумою та курсом (якщо не UAH)
- В edit-drawer: live-перерахунок суми в гривнях при зміні суми / курсу
...
**Призначення:** звітність ФОП 3-ї групи — єдиний податок (ЄП) та військовий збір (ВЗ).

**Ставки:** ЄП = 5%, ВЗ = 1% від доходу.

**Звітні періоди (наростаючим з початку року):**
| № | Назва | Місяці |
|---|---|---|
| 1 | I квартал | 1–3 |
| 2 | Півріччя | 1–6 |
...
**Джерела доходу:**
- `invoices` де `pay_status = 'Оплачено'` — за полем `pay_date` (dd.mm.yyyy)
- `app_payments` — всі записи, за полем `pay_date` (dd.mm.yyyy)

**Відображення:**
- Вкладки по роках (найновіший активний за замовчуванням)
- KPI-картки: дохід за рік, ЄП, ВЗ, разом
- Таблиця: квартальний дохід | наростаючим | ЄП кварт. | ВЗ кварт. | **Разом (наростаючим)** ← вноситься в декларацію
- Картки-мініатюри по кварталах внизу

---

### 5.10 НБУ-прив'язка договорів

**Поля в `contracts`:** `nbu_rate` (базовий курс), `nbu_tracking` (0/1), `nbu_threshold_pct` (поріг %).

**Логіка `nbu_client.py`:**
- `fetch_from_api()` — прямий запит до `bank.gov.ua`
- `get_rate()` — повертає курс з кешу або API (TTL 6 год)
- `compute_status(contracts)` — розраховує δ% для кожного tracked-договору, повертає `tracked` + `alerts`
- `get_rate_safe()` — ніколи не кидає виняток (fallback при мережевій помилці)

**Фоновий refresh (main.py):**
```python
...
```
Запускається через `asyncio.create_task()` при старті FastAPI.

---

### 5.11 Сортування таблиць

Всі таблиці CRM (клієнти, договори, рахунки, акти, витрати, оплати, LiqPay) підтримують **клієнтське сортування по заголовку колонки**.

**Реалізація (base.html):** глобальна функція `initSortable(tableId)`:
- Клік по `<th>` → сортування ↑/↓ (повторний клік — реверс)
- `<th data-nosort>` — колонка виключена з сортування (чекбокси, кнопки "Дії")
- `<td data-sort="...">` — явне числове значення для сортування (наприклад `data-sort="{{ e.amount_uah }}"`)
- Розпізнає формати: `dd.mm.yyyy` (дата), `N NNN,NN` (число), рядок (localeCompare 'uk')

---

### 5.12 Шаблони документів (`/templates`)
...
|---|---|---|
| Дашборд | ✅ Повністю | KPI, графік, активність, прогнози, НБУ-блок |
| Клієнти | ✅ Повністю | CRUD + Excel + сортування |
| Договори | ✅ Повністю | CRUD + MRR/ARR + таблиця/картки + прив'язка НБУ + сортування |
| Рахунки (список) | ✅ Повністю | Фільтри, drawer, нагадування, сортування |
| Рахунки (генерація PDF) | ✅ Повністю | Доступ + Погодинний |
| Акти (список) | ✅ Повністю | Фільтри, статуси, сортування |
| Акти (генерація PDF) | ✅ Повністю | Кастомні шаблони |
| Нагадування | ✅ Повністю | Шаблон + генерація + позначка |
| Витрати | ✅ Повністю | CRUD + мультивалютність + категорії з БД + edit drawer + сортування |
| Звірка з банком | ✅ Повністю | Monobank API + auto-match |
| LiqPay / App Payments | ✅ Повністю | CRUD + фільтри + сортування |
| Шаблони | ✅ Повністю | Upload + редактор + призначення до договору |
| **Податки** | ✅ Повністю | ЄП 5% + ВЗ 1%, наростаючим, вкладки по роках |
| **НБУ-моніторинг** | ✅ Повністю | Фоновий refresh, кеш, алерти на дашборді |
| Excel-експорт | ✅ Повністю | Всі модулі |
...
7. **Нумерація** — `RAH-*` і `ACT-*` використовують MAX-based генерацію (стійко до видалень). При паралельному записі SQLite UNIQUE constraint захищає від колізій.
...
*Документ оновлено на основі аудиту кодової бази станом на 14.06.2026.*  
*Зміни v1.2: НБУ-моніторинг договорів, сторінка Податки (ФОП), мультивалютні витрати, категорії витрат з БД, сортування таблиць.*  
*Зміни v1.3: MAX-based нумерація рахунків/актів (стійко до видалень); підтримка слешів у номері договору (path-параметр); статуси договору приведені до 4 канонічних значень (Активний/Призупинено/Завершено/Чернетка); прибрано кнопки «Переглянути документ» та «Підписати» зі списків.*
